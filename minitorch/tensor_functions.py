...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        tmp = backend.mul_zip(grad_output, sig)
        one_tensor = ones(tmp.shape)
        result = backend.mul_zip(tmp, backend.add_zip(one_tensor, backend.neg_map(sig)))
        return result
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        return (
            minitorch.Tensor(
                grad_output._tensor.permute(*rev_order),
